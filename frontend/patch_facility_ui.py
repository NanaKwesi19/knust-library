...
export default function FacilityManagement() {
  ...
  const updateMutation = useMutation({
    ...
    onError: (err: any) => {
      addToast(`Error: ${err.response?.data?.error || 'Failed to update room'}`);
    }
  });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editingRoomId) {
      updateMutation.mutate({ id: editingRoomId, payload });
    } else {
  ...
  return (
    <div className="space-y-6">
            ...
            <Card key={room.id} className="p-5 flex flex-col justify-between">
              <div>
                <div className="flex items-center justify-between mb-3">
                  <h3 className="font-bold text-lg text-slate-900 flex items-center gap-2 truncate pr-2">
                    <Building2 className="w-5 h-5 text-[#7A1C2C] shrink-0" /> <span className="truncate">{room.roomNumber}</span>
                  </h3>
                  <span className="bg-slate-100 text-slate-600 px-2 py-1 rounded text-xs font-bold flex items-center gap-1 shrink-0">
                    <Users className="w-3 h-3" /> {room.capacity}
                  </span>
                </div>
                <p className="text-sm text-slate-500 flex items-center gap-1 mb-2">
                  <MapPin className="w-4 h-4 shrink-0" /> {room.location || 'N/A'}
                </p>
              ...
              <div className="mt-4 pt-3 border-t border-slate-100 flex justify-end gap-2">
                <button
                  ...
                  className="text-slate-400 hover:text-[#7A1C2C] p-2 transition-colors"
                >
      ...
      <Modal isOpen={isModalOpen} onClose={closeModal} title={editingRoomId ? "Edit Study Room" : "Create Study Room"}>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-xs font-bold text-slate-500 uppercase tracking-wider block mb-1">Room Name / Number *</label>
              <input required value={formData.roomNumber} onChange={e => setFormData({...formData, roomNumber: e.target.value})} className="w-full px-3 py-2 border rounded-xl" placeholder="e.g. Conference Room A" />
            </div>
            <div>
          ...
          <div className="flex justify-end pt-4 border-t border-slate-100">
            <Button type="submit" isLoading={editingRoomId ? updateMutation.isPending : createMutation.isPending}>
